 ...
 */
    ...
    connect(client, &TCPclient::sig_getAvailSize, this, &MainWindow::DisplayFreeSpace);
    ...
    connect(client, &TCPclient::sig_sendFreeSize, this, &MainWindow::DisplaySuccess);
}
...
{
    ui->tb_result->append("Данные (" + ui->le_data->text() + ") были отправленны на сервер, сервер вернул ответ: " + replyString);
}
...
void MainWindow::DisplayError(uint16_t error)
{
    switch (error) {
        case ERR_ZERO_LEN:{
            ui->tb_result->append("Ошибка: Пакет данных нулевой длинны.");
            break;
        }
        case ERR_NO_FREE_SPACE:{
            ui->tb_result->append("Ошибка: На сервере кончилось свободное место.");
            break;
        }
        case ERR_NO_FUNCT:{
            ui->tb_result->append("Ошибка: Такой запрос не реализован в текущей версии.");
            break;
        }
        default:
            break;
...
void MainWindow::DisplaySuccess(uint16_t typeMess)
{
    switch (typeMess) {
    case CLEAR_SPACE:{
        ui->tb_result->append("Память на сервере освобождена успешно.");
        break;
    }
    default:
        break;
...
void MainWindow::on_pb_request_clicked()
{
   ...
   switch (ui->cb_request->currentIndex()){
        ...
        case 3:{
           ...
           QString input_data = ui->le_data->text();

           header.len = input_data.toUtf8().size();
           client->SendData(header, input_data);
